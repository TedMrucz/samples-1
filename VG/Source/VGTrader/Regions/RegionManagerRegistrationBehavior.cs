	...
	public class RegionManagerRegistrationBehavior : RegionBehavior, IHostAwareRegionBehavior
	{
		...
		public DependencyObject HostControl
		{
			...
			set
			{
				if (IsAttached)
				{
					throw new InvalidOperationException("HostControl Cannot Be Set After Attach");
				}
		...
		private IRegionManager FindRegionManager(DependencyObject dependencyObject)
		{
			...
			if (parent == null)
			{
				var targetElement = VisualTreeHelper.GetParent(dependencyObject) as FrameworkElement;
				if (targetElement != null)
					return FindRegionManager(targetElement);
			}
			else
			{
